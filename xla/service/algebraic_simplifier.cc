...
// Remove upcasting converts before a binary op when the result will be downcast
// afterwards.
//
// convert<TS>(bin_op<TL>(convert<TL>(data1<TS>),
//                        convert<TL>(data2<TS>)))
//  where TS is a smaller type than TL (ex, TS=fp16, TL=fp32)
// ->
// bin_op<TS>(data1<TS>, data2<TS>)
//
// See algebraic_simplifier_alive2_proofs.md for tests which verify validity of
